...
- `graphql`: A query language and runtime for executing those queries with existing data.
- `@apollo/server`: A community-maintained, open-source GraphQL server.
- `@graphql-tools/merge`: A package to merge schemas and type definitions in GraphQL.
- `bcryptjs`: Library to help hash passwords.
- `connect-mongodb-session`: MongoDB session store for Connect and Express.
- `dotenv`: Loads environment variables from a `.env` file into `process.env`.
- `graphql-passport`: A Passport.js authentication strategy for GraphQL.
...
- `graphql`: A query language and runtime for executing those queries with existing data.
- `@apollo/client`: Apollo Client is a fully-featured caching GraphQL client with integrations for React.
...
- `react-hot-toast`: A lightweight, customizable toast notification library for React.
- `tailwind-merge`: A utility for merging Tailwind CSS classes.
- `@tailwindcss/aspect-ratio`: A plugin for Tailwind CSS to generate aspect ratio utilities.
- `clsx`: A tiny utility for constructing classNames strings conditionally in JavaScript.
- `chart.js`: A simple yet flexible JavaScript charting library for designers & developers.
- `react-chartjs-2`: A React wrapper for Chart.js 2, providing easy integration of charts into React applications.
- `mini-svg-data-uri`: A lightweight library to convert SVG strings to data URIs.
